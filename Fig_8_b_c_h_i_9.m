...
[SpecA,Atau,Af] = zALCT(Sig0,Hz,Nh,Nc,Num);
...
for i = 1 : Num
    ...
    Spec = Spec'; %
%     figure;imagesc(Atau,Af,abs(Spec)); axis([0 2 0 100]); axis xy
    %     [Spec,Af] = STFT(real(Sig0(:)),Hz,Nfrebin,window); %STFT
...
plot(t,real(rrcon(1,:)),'--c');ylim([-2.5 3]);
...
plot(t,real(rrcon(2,:)),'--c');ylim([-3 4]);
xlabel('Time (s)'); ylabel('Amplitude');legend('Real','Reconstructed','Orientation','horizontal');
...
% theta_dot = if1;
% q1 = real(rrcon(1,:));
% q1_dot = gradient(q1)/Hz;
% E1 = 0.5*(q1_dot.^2 + theta_dot.^2.*q1.^2);
% figure;plot(t,E1)
% xlabel('Time (s)'); ylabel('Energy')

%% After Rebuttal 2, from: An underdetermined blind source separation method with application to modal identification
[m, T] = size(Sig');
