...
Sigr = exp(2*pi*1i*(10*t));                
[Spec2,~] = STFT(real(Sigr(:)),Hz,Nfrebin,window);

N = length(t);
n = NaN(1,N);
hop = 8;                                                                   % Added for rebuttal
deltatime = 23;
for p = deltatime : hop: N-(deltatime-1)
    Spec1 = 0*Spec0;
    Spec3 = 0*Spec2;
    pii = p-(deltatime-1); pf = p + (deltatime-1);
...
plot(t,iniIF(1,:),'--c','LineWidth',2);plot(t,iniIF(2,:),'--r','LineWidth',2); xlabel('Time (s)'); ylabel('Frequency (Hz)');axis([0 2 0 110]);
...
subplot(223)
plot(t, real(Sig1'-rrcon(1,:)), 'r'); ylim([-1.5 2]); xlabel('Time (s)');ylabel('Amplitude');legend('Error')

subplot(222)
plot(t,real(Sig2), 'color', 'k'); xlabel('Time (s)');hold on;
plot(t,real(rrcon(2,:)),'--c');ylim([-.8 1.1])
xlabel('Time (s)'); ylabel('Amplitude');legend('Real','Reconstructed','Orientation','horizontal');

subplot(224)
plot(t, real(Sig2'-rrcon(2,:)), 'r'); ylim([-.8 1.1]); xlabel('Time (s)');ylabel('Amplitude');legend('Error')

set(findall(gcf,'-property','FontSize'),'FontSize',12, 'FontName', 'Times New Roman')

% %% To explore from: An underdetermined blind source separation method with application to modal identification
% [m, T] = size(Sig');
% 
% L = floor(T/4);
% L= L+1-rem(L,2);
...
% subplot(221)
% plot(t,real(Sig1), 'color', 'k'); xlabel('time (s)');hold on
% plot(t,source(1,:),'--c');ylim([-1.5 2])
% xlabel('Time (s)'); ylabel('Amplitude');legend('Real','Reconstructed','Orientation','horizontal');
% 
% subplot(223)
% plot(t, real(Sig1')-source(1,:), 'r'); ylim([-1.5 2]); xlabel('Time (s)');ylabel('Amplitude');legend('Error')
% 
% subplot(222)
% plot(t,real(Sig2), 'color', 'k'); xlabel('Time (s)');hold on;
% plot(t,source(2,:),'--c');ylim([-.8 1.1])
% xlabel('Time (s)'); ylabel('Amplitude');legend('Real','Reconstructed','Orientation','horizontal');
% 
% subplot(224)
% plot(t, real(Sig2')-source(2,:), 'r'); ylim([-.8 1.1]); xlabel('Time (s)');ylabel('Amplitude');legend('Error')
% 
% set(findall(gcf,'-property','FontSize'),'FontSize',12, 'FontName', 'Times New Roman')