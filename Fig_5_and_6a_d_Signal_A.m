...
subplot(232);imagesc(Atau,Aw,abs(TFR.')/max_tfr);axis xy;
...
caxis([0 1]);
set(cbar, 'Position', [0.56, 0.48, 0.2, 0.02]);
...
subplot(233);imagesc(Atau,Aw,abs(TFR.')/max_tfr);axis xy;
...
subplot(222);plot(t,real(Sig2),'k');hold on;plot(t,real(mode2),'--c');ylim([-.8 1.1])
xlabel('Time (s)');ylabel('Amplitude');legend('Original','Reconstructed','Orientation','horizontal');
subplot(224);plot(t,real(Sig2-mode2),'r');xlabel('Time (s)');ylabel('Amplitude');
ylim([-.8 1.1]);xlabel('Time (s)');ylabel('Amplitude');legend('Error')
